...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const sqrtX = x1 ** 2 - 2 * x2 * x1 + x2 ** 2;
  const sqrtY = y1 ** 2 - 2 * y2 * y1 + y2 ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modulX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modulY = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = scalar / (modulX * modulY);
...
function getLastDigit(value) {
  const lastChar = Math.floor(value) % 10;
  return lastChar;
}
...
function roundToPowerOfTen(num, pow) {
  return Math.round(Math.floor(num) / 10 ** pow) * 10 ** pow;
}
